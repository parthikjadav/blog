...
export const PostInputZ = z.object({
  ...
  readingTime: z.number().int('Reading time must be an integer').min(0, 'Reading time cannot be negative').max(3600, 'Reading time must be less than 3600 minutes'),
  ...
  categorySlug: z.string().min(1, 'Category slug is required').max(60, 'Category slug must be 60 characters or less'),
  tags: z.array(z.string().min(1).max(60)).max(30, 'Maximum 30 tags allowed').default([]),
  keywords: z.array(z.string().min(1).max(40)).max(40, 'Maximum 40 keywords allowed').default([]),

  featuredImage: z.string().url('Featured image must be a valid URL').optional(),
  featuredImageAlt: z.string().max(160, 'Featured image alt text must be 160 characters or less').optional(),
})

/**
 * Zod schema for validating an array of post inputs
 */
export const BulkPostsZ = z.array(PostInputZ).min(1, 'At least one post is required')
